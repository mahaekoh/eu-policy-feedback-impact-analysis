...
# ── vLLM helpers (duplicated from summarize_documents.py per codebase convention) ──
...
def summarize_single_text(text, prompt_prefix, combine_prefix, chunk_size,
    ...
    for ci, chunk in enumerate(chunks):
        prefill = build_prefill(encoding, chunk, prompt_prefix, reasoning_effort)
        if prefill is None:
    ...
    while len(current) > 1:
        ...
        for gi, group in enumerate(groups):
            if len(group) == 1:
                ...
                continue
            combined = "\n\n".join(group)
            prefill = build_prefill(encoding, combined, combine_prefix, reasoning_effort)
            if prefill is None:
                single_pass_indices.append(gi)
                continue
            prompts.append(prefill)
...
def process_initiative(initiative, llm, sampling_params, encoding, reasoning_effort,
    ...
    for feedback_id in cluster_assignments:
        ...
        for ci, chunk in enumerate(chunks):
            prefill = build_prefill(encoding, chunk, FEEDBACK_SUMMARY_PREFIX, reasoning_effort)
            if prefill is None:
                ...
                for feedback_id, parts in current_parts.items():
                    ...
                    for gi, group in enumerate(groups):
                        if len(group) == 1:
                            continue
                        combined = "\n\n".join(group)
                        prefill = build_prefill(encoding, combined, FEEDBACK_COMBINE_PREFIX, reasoning_effort)
                        if prefill is None:
                            continue
        ...
        for label in labels_at_depth:
            ...
            for gi, group in enumerate(groups):
                if len(group) == 1:
                    continue
                combined = "\n\n".join(group)
                prefill = build_prefill(encoding, combined, CLUSTER_COMBINE_PREFIX, reasoning_effort)
                if prefill is None:
                    continue
